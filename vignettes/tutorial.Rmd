...
As the code reveals, the function loops though the values that are returned by
...
Let's have a look at the first functions and see...
...
origin of each column by postfixing the column related to the name of the data 
frame that it came from.
...
This function checks if all elements in a list are identical to each other. 
It may be useful when checking if different versions of a function (that may,
e.g., be created during code cleaning) return exactly the same, when being 
...
Take care when comparing floating point numbers! It depends on how they were
calculated if floating point numbers are assumed to be equal or not, as shown in
