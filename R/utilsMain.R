...
#' 
#' If x is numeric and all values are whole numbers use the sequence between
#'   the smallest and the highest value
#' 
#' @param x numeric or character vector
#' @param step step to be used in sequence generation
#' 
#' @return if \code{x} is numeric the sequence between the lowest and highest value
#'   in \code{x} with the given \code{step} is returned. If \code{x} is a
...
#'   
#' 
defaultLevels <- structure(function # Default factor levels
### If x is numeric and all values are whole numbers use the sequence between
### the smallest and the highest value
(
  x,
...
)
{
  ...
  sort(unique(x))
  ### if \code{x} is numeric the sequence between the lowest and highest value
  ### in \code{x} with the given \code{step} is returned. If \code{x} is a
  ### vector of character strings, the sorted unique values are returned.
}, ex = function() {
  defaultLevels(c(1, 3, 4, 5, 4))
...
#' @param first if \code{TRUE} (default) the \code{className} is prepended to the vector
#'   of existing class names, otherwise appended to the end of the vector
#' @param dbg if \code{TRUE} (default), debug messages are shown.
#' @return \code{x} with the \code{class} attributed extended by \code{className}
#'   (only if \code{x} did not yet inherit from \code{className})
#' 
addClass <- function # add a class to an object
### Extend the class attribute by a given class name
(
  x,
  ### R object
  className,
  ### name of the class to be added to \code{x}
  first = TRUE,
  ### if \code{TRUE} (default) the \code{className} is prepended to the vector
  ### of existing class names, otherwise appended to the end of the vector
  dbg = FALSE
)
{
  ...
  x
  ### \code{x} with the \code{class} attributed extended by \code{className}
  ### (only if \code{x} did not yet inherit from \code{className})
}
...
#' @param count if \code{TRUE} (the default is \code{FALSE}) the number of replaced
#'   values is returned in the attributes \code{count}
#' 
...
#'   
#' 
defaultIfNULL <- structure(
  function # default value if object is NULL
  ### return the given object or a default value if the object is NULL
  (
    x,
    ### R object to be checked for NULL
    default,
    ### default value that is returned if \code{x} is NULL
    count = FALSE
    ### if \code{TRUE} (the default is \code{FALSE}) the number of replaced
    ### values is returned in the attributes \code{count}
  )
  {
    ##seealso<< \code{\link{defaultIfNA}, \link{defaultIfZero}}
    ...
    ### \code{x} if \code{x} is not NULL and \code{default} otherwise
  }, ex = function() {
    defaultIfNULL(NULL, "default") # returns the default value
    defaultIfNULL("actual", "default") # returns the "actual" value
  })
...
#'   
#' 
defaultIfNA <- structure(
  function # default value if object is NA
  ### return the given object or a default value if the object is NA
  (
    x,
    default,
    ### default value that is returned if \code{x} is NA
    count = FALSE
    ### if \code{TRUE} (the default is \code{FALSE}) the number of replaced
    ### values is returned in the attributes \code{count}
  )
  {
    ...
    ### \code{x} if \code{x} is not NA and \code{default} otherwise
  }, ex = function() {
    defaultIfNA(NA, "default") # returns the default value
    defaultIfNA("actual", "default") # returns the "actual" value
  })
...
#'   values is returned in the attributes \code{count}
#' 
#' @return \code{x} if \code{x} is not 0 and \code{default} otherwise. The returned
#'   object has an attribute \code{count} containing the number of values
...
#'   
#' 
defaultIfZero <- structure(
  function # default value if object is 0 (zero)
  ### return the given object or a default value if the object is 0 (zero)
  (
    x,
    default,
  ...
  )
  {
    ...
    ### \code{x} if \code{x} is not 0 and \code{default} otherwise. The returned
    ### object has an attribute \code{count} containing the number of values
    ### that have been set to the default value.
  }, ex = function() {
    defaultIfZero(c(1, 2, 0, 4, 5, 0, 6), NA) # returns the default value (NA)
...
#' Restores object attributes
#' 
#' Restores given attributes that are not object attributes any more
#' 
#' @param x object
#' @param attribs former attributes of x (as retrieved by attributes(x)) to be restored
#' 
...
#' @param do.stop if \code{TRUE} (default) an error is raised if the attribute does not
#'   exist.
#' 
getAttribute <- function # safely get the attribute of an object
### safely get the attribute of an object. An error is given if the attribute
### does not exist (and do.stop = TRUE)
(
  x,
  ...
  do.stop = TRUE
  ### if \code{TRUE} (default) an error is raised if the attribute does not
  ### exist.
)
{
  ...
  if (do.stop && ! attributeName %in% attributeNames) {
    stop("No such attribute: ", hsQuoteChr(attributeName), " in ",
