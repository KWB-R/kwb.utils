...
#' @param tolerance tolerance, i.e. accepted difference between values in \code{x} and 
#'   \code{y}. Default: 1e-12
#' 
#' @return vector of logical
#' 
almostEqual <- function # compare numerical vectors with tolerance
### compare numerical vectors accepting a tolerance
(
  x, 
  ### vector of numerical
  ...
  ### vector of numerical
  tolerance = 1e-12
  ### tolerance, i.e. accepted difference between values in \code{x} and 
  ### \code{y}. Default: 1e-12
)
{
...
#' @param criteria vector of character containing conditions, in which the column names of
#'   \code{Data}, e.g. \code{A} can appear unquoted, e.g. \code{"A == 'x'"}
#' @param na.to.false if \code{TRUE} (the default is \code{FALSE}) NA in the resulting vector
...
#'   \code{details} 
#' @param dbg if TRUE (default) for each criterion in \code{criteria} it is shown for 
#'   how many rows in \code{Data} the criterion is TRUE and for how many rows
#'   it is FALSE
#' 
...
)
{
  ...
  for (criterion in criteria) {
    ...
    catIf(dbg, sprintf("  is TRUE for %7d rows (%5.1f %%),\n", 
                       sum.true, percentage(sum.true, basis = N)))
    catIf(dbg, sprintf("    FALSE for %7d rows (%5.1f %%) and\n", 
                       sum.false, percentage(sum.false, basis = N)))
    catIf(dbg, sprintf("       NA for %7d rows (%5.1f %%).\n", 
                       sum.na, percentage(sum.na, basis = N)))
    catIf(dbg, sprintf("  Selected rows now: %d\n", sum(selected, na.rm = TRUE)))
  }
  ...
  if (sum.na > 0) {
    ...
    if (na.to.false) {
      ...
      warning("There are ", sum.na, " NAs in the returned vector. This will lead ", 
...
#' @param method Select one of two methods. 1: check if the length of unique elements is
#'   equal to one, 2: check if all elements are equal to the first element.
#' 
#' @return \code{TRUE} if all elements in \code{x} are equal to each other, otherwise 
#'   \code{FALSE}
#' 
allAreEqual <- function # are all elements in x the same?
### are all elements in x the same?
(
  x,
  ### vector of elements to be compared
  method = 1
  ### Select one of two methods. 1: check if the length of unique elements is
  ### equal to one, 2: check if all elements are equal to the first element.
)
{
  if (method == 1) {
  ...
  ### \code{TRUE} if all elements in \code{x} are equal to each other, otherwise 
  ### \code{FALSE}
}
...
#'   \code{FALSE}
#' 
allAreIdentical <- function # Are all list elements identical to each other?
### Are all list elements identical to each other?
(
  x
  ### a list
)
{
  ...
  if (! is.list(x)) {
    stop(name.x, " is assumed to be a list! Use allAreEqual() to compare all ", 
  ...
  ### \code{TRUE} if all elements in \code{x} are identical, otherwise 
  ### \code{FALSE}
}
...
#' isNullOrEmpty
#' 
#' @param x object to be tested for NULL or being empty (vector or list of length 0 or 
#'   data frame with no rows)
#' 
#' @return TRUE if x is NULL or x is a vector of length 0 or x is a data frame with
#'   no rows.
#' 
